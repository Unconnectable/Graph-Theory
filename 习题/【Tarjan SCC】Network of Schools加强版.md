...
## 大意:1.图G=(V,E)选几个点可以到达所有的点 2.连多少条边可以让任意一个点出发到达其他所有点

## 思路:1.Tarjan 跑一遍求SCC  那些出度为0的点就是出发的所有点 即din==0的点的数量  2.计算dout==0的点的数量  和din==0的点的数量取max 因为把那些din==0或者dout==0的scc点连接起来就能让所有图联通  ，特判一下 如果dout==0的数量为1， 说明这个图只有一个scc  那么这个图就是联通的，不需要加边，输出0
